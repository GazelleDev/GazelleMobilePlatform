...
function resolveRequestUserId(context: RequestUserContext | undefined) {
  ...
  return context?.userId ?? defaultLoyaltyUserId;
}
...
async function resolveOrderUserId(params: {
  ...
  const resolvedUserId = resolveRequestUserId(requestUserContext);
  if (isServiceError(resolvedUserId)) {
    return resolvedUserId;
  }

  await repository.setOrderUserId(orderId, resolvedUserId);
  return resolvedUserId;
}
...
export async function cancelOrder(params: {
  ...
  let notificationUserId = await deps.repository.getOrderUserId(orderId);
  if (!notificationUserId) {
    const resolvedRequestUserId = resolveRequestUserId(requestUserContext);
    if (isServiceError(resolvedRequestUserId)) {
      return { error: resolvedRequestUserId };
    }
    notificationUserId = resolvedRequestUserId;
    await deps.repository.setOrderUserId(orderId, resolvedRequestUserId);
  }
