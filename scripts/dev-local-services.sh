...
REPORTING_UPSTREAM_HOST="${REPORTING_UPSTREAM_HOST:-127.0.0.1}"
...
start_service "orders:3001" env PORT=3001 HOST="${BIND_HOST}" GATEWAY_INTERNAL_API_TOKEN="${GATEWAY_INTERNAL_API_TOKEN:-}" CATALOG_SERVICE_BASE_URL="http://${CATALOG_UPSTREAM_HOST}:3002" ORDERS_INTERNAL_API_TOKEN="${ORDERS_INTERNAL_API_TOKEN:-}" LOYALTY_INTERNAL_API_TOKEN="${LOYALTY_INTERNAL_API_TOKEN:-}" NOTIFICATIONS_INTERNAL_API_TOKEN="${NOTIFICATIONS_INTERNAL_API_TOKEN:-}" VALKEY_URL="${VALKEY_URL}" pnpm -C "${ROOT_DIR}" --filter @lattelink/orders dev
...
start_service "notifications:3005" env PORT=3005 HOST="${BIND_HOST}" GATEWAY_INTERNAL_API_TOKEN="${GATEWAY_INTERNAL_API_TOKEN:-}" NOTIFICATIONS_INTERNAL_API_TOKEN="${NOTIFICATIONS_INTERNAL_API_TOKEN:-}" VALKEY_URL="${VALKEY_URL}" pnpm -C "${ROOT_DIR}" --filter @lattelink/notifications dev
start_service "reporting:3006" env PORT=3006 HOST="${BIND_HOST}" GATEWAY_INTERNAL_API_TOKEN="${GATEWAY_INTERNAL_API_TOKEN:-}" pnpm -C "${ROOT_DIR}" --filter @lattelink/reporting dev
...
start_service \
  ...
  REPORTING_SERVICE_BASE_URL="http://${REPORTING_UPSTREAM_HOST}:3006" \
...
if [[ "${START_NOTIFICATIONS_DISPATCH_WORKER:-1}" == "1" ]]; then
  start_service "notifications-dispatch-worker" env NOTIFICATIONS_SERVICE_BASE_URL="http://127.0.0.1:3005" NOTIFICATIONS_INTERNAL_API_TOKEN="${NOTIFICATIONS_INTERNAL_API_TOKEN:-}" pnpm -C "${ROOT_DIR}" --filter @lattelink/notifications-dispatch-worker dev
fi
